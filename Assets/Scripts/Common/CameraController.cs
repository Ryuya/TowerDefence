...
namespace Assets.Scripts.Common
{
	...
	private GameObject focusObj = null;
		...
		void Update ()
		{
			// マウス関係のイベントを関数にまとめる
			this.mouseEvent ();
			//this.transform.position = new Vector3(this.transform.position.x,Mathf.Clamp(transform.position.y,11,50),this.transform.position.z);
		...
		void mouseWheelEvent (float delta)
		{
			...
			if ((this.focusObj.transform.position.y + post.y) > 11f && (this.focusObj.transform.position.y + post.y) < 50f) {
